...
const TripManagement: React.FC = () => {
    ...
    const handleExportCSV = () => {
        const dataToExport = tripsWithData.map(trip => {
            ...
            const hasStartPhoto = !!(trip.startOdometerPhotoUrl || (trip as { startOdometerPhotoId?: string }).startOdometerPhotoId);
            const hasEndPhoto = !!(trip.endOdometerPhotoUrl || (trip as { endOdometerPhotoId?: string }).endOdometerPhotoId);
            const checkOutTime = resolveCheckOutTime(trip);
            const checkInTime = resolveCheckInTime(trip);

            return {
                'Trip Number': trip.tripNumber,
                'Duty Slip Number': trip.dutySlipNumber || 'N/A',
                'Status': trip.status,
                ...
                'Check In Date': formDateFromIso(trip.plannedCheckInTime ?? trip.actualCheckInTime),
                'Check In Time': formTimeFromIso(trip.plannedCheckInTime ?? trip.actualCheckInTime),
                'Check Out Date': formDateFromIso(trip.plannedCheckOutTime ?? trip.actualCheckOutTime),
                'Check Out Time': formTimeFromIso(trip.plannedCheckOutTime ?? trip.actualCheckOutTime),
                'Start Odometer (km)': csvNumber(trip.plannedStartOdometer ?? trip.actualStartOdometer),
                'End Odometer (km)': csvNumber(trip.plannedEndOdometer ?? trip.actualEndOdometer),
                'Check-in (recorded)': csvDateTime(checkInTime),
                'Check-out (recorded)': csvDateTime(checkOutTime),
                'Start Odometer (recorded)': csvNumber(resolveStartOdometer(trip)),
                'End Odometer (recorded)': csvNumber(resolveEndOdometer(trip)),
                'Start Photo URL': hasStartPhoto ? apiService.getStartPhotoUrl(trip.id) : (trip.startOdometerPhotoUrl || 'N/A'),
                'End Photo URL': hasEndPhoto ? apiService.getEndPhotoUrl(trip.id) : (trip.endOdometerPhotoUrl || 'N/A'),
